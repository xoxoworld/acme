...
if (slideWrapper) {
  ...
  /* pager 생성 및 클릭 이벤트 바인딩 */
  const pagerContainer = document.createElement("div");
  pagerContainer.classList.add("pager");

  const pagerBullets = [];

  for (let i = 0; i < slideCount; i++) {
    const pagerBullet = document.createElement("span");
    if (i === 0) {
      pagerBullet.classList.add("active");
    }
    pagerBullet.addEventListener("click", () => {
      moveSlide(i);
    });
    pagerContainer.appendChild(pagerBullet);
    pagerBullets.push(pagerBullet);
  }

  slideWrapper.appendChild(pagerContainer);
  ...
  */
  function moveSlide(num) {
    ...
    // pager active 클래스 업데이트
    pagerBullets.forEach((bullet, idx) => {
      if (idx === num) {
